...
const Chatbot = () => {
    const [messages, setMessages] = useState([{ text: 'Hello! I am your friendly chatbot.', sender: 'bot' }, { text: 'Hi, what can I do for you?', sender: 'user' }])
    ...
    return (
        <Paper elevation={5} className={`w-[350px] mt-3 mr-2 ${show ? "h-[0px]" : "h-[590px] bg-transparent"}`} sx={{ borderRadius: '30px 30px 0 0' }}>
            <Grid container direction="row" className="bg-[#304ffe] h-[140px] text-white flex justify-center items-center" sx={{ borderRadius: '30px 30px 0 0' }}>
                ...
                <IconButton className={show ? "hidden" : ""} style={{ position: "absolute", right: 40, top: 15 }} color="inherit" aria-label="down arrow" onClick={handleShow}>
                    <HomeIcon />
                </IconButton>
                <IconButton className={show ? "hidden" : ""} style={{ position: "absolute", right: 10, top: 15 }} color="inherit" aria-label="down arrow" onClick={handleShow}>
                    <RemoveIcon />
                </IconButton>
            </Grid>
            <Grid container className={`h-[400px] bg-gray-100 ${show ? "hidden" : ""}`}>
                <div style={{display: "flex", flexDirection: "column", height: "100%", width: "100%"}}>
                    {messages.map((message, index) => (
                        <Grid container key={index.toString()} direction={message.sender === 'bot' ? 'row' : 'row-reverse'} alignItems="center">
                            {message.sender === 'bot' && (
                                <Avatar alt="Bot Avatar" src="/images/Admin/avatar1.jpg" />
                            )}
                            <Paper elevation={3} className={`m-2 p-2 ${message.sender === 'bot' ? 'bg-white' : 'bg-blue-200'}`}>
